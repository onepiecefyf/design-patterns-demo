...
(function ($) {
  ...
  $.fn.InputSpinner = function (options) {
    ...
    var html = '<div class="input-group ' + config.groupClass + '">' +
        ...
        '<button style="min-width: ' + config.buttonsWidth
        + '" class="btn btn-decrement ' + config.buttonsClass
        + '" type="button">' + config.decrementButton + '</button>' +
        '</div>' +
        '<input type="text" style="text-align: ' + config.textAlign
        + '" class="form-control ' + config.inputClass + '"/>' +
        '<div class="input-group-append">' +
        '<button style="min-width: ' + config.buttonsWidth
        + '" class="btn btn-increment ' + config.buttonsClass
        + '" type="button">' + config.incrementButton + '</button>' +
        '</div>' +
        '</div>'
    ...
    this.each(function () {
      ...
      function calcStep(step) {
        ...
        $input.val(numberFormat.format(value))
        $original.val(Math.round(value * Math.pow(10, decimals)) / Math.pow(10,
            decimals))
        dispatchChangeEvents($original)
      }
