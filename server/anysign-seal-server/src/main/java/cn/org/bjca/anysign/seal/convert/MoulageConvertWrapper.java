...
public class MoulageConvertWrapper {
    ...
    public static void verification(SignImage request) {
        ...
        Assert.notNull(request.getFileTransmissionType(), StatusConstants.FILETRANSMISSIONTYPE_NOTNULL, null, "生成印章");
        Assert.notNull(request.getMoulage(), StatusConstants.IMAGE_PARAMETER_NOTNULL, null, "生成印章");
        Assert.notNull(request.getMoulage().getBorderWeight(), StatusConstants.BORDERWEIGHT_NOTNULL, null, "生成印章");
        Assert.notNull(request.getMoulage().getFileType(), StatusConstants.FILETYPE_NOTNULL, null, "生成印章");
        Assert.notNull(request.getMoulage().getSealWidth(), StatusConstants.SEALWIDTH_NOTNULL, null, "生成印章");
        Assert.notNull(request.getMoulage().getSealHeight(), StatusConstants.SEALHEIGHT_NOTNULL, null, "生成印章");
        Assert.notNull(request.getMoulage().getBorderWeight(), StatusConstants.BORDERWEIGHT_NOTNULL, null, "生成印章");
        Assert.notNull(request.getMoulage().getSealColor(), StatusConstants.SEALCOLOR_NOTNULL, null, "生成印章");
        Assert.notNull(request.getMoulage().getSealshape(), StatusConstants.SEALSHAPE_NOTNULL, null, "生成印章");
        Assert.notNull(request.getMoulage().getFontFamily(), StatusConstants.FONTFAMILY_NOTNULL, null, "生成印章");
    ...
    public static void riderVerification(List<RidersBean> ridersBeens) {
        ridersBeens.forEach(ridersBean -> {
            Assert.notNull(ridersBean.getAppendixContent(), StatusConstants.APPENDIXCONTENT_NOTNULL, null, "生成印章");
    ...
    private static List<Map<String, Object>> getTxtDescs(SignImage request) {
        ...
        for (RidersBean ridersBean : request.getMoulage().getRiders()) {
            Map<String, Object> topTxt = SealImage.buildText(ridersBean.getAppendixContent(), request.getMoulage().getSealColor(),
    ...
    public static void pdfSignatureverification(SignatureRequest request) {
        ...
        Assert.notNull(request.getFileTransmissionType(), StatusConstants.FILETYPE_NOTNULL, null, "PDF签章");
    ...
    private static void sealPositionVerification(SealPosition sealPosition) {
        ...
        switch (sealPosition.getPositionType()) {
            case KEYWORD:
                ...
                break;
            case COORDINATE:
                Assert.notNull(sealPosition.getXyzRuleInfo(), StatusConstants.XYZRULEINFO_ERROR, null, "PDF签章");
                break;
            case CROSS_PAGE:
                Assert.notNull(sealPosition.getAcrossPage(), StatusConstants.ACROSSPAGE_ERROR, null, "PDF签章");
