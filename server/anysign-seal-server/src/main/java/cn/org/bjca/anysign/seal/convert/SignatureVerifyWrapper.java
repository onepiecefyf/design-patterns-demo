...
public class SignatureVerifyWrapper {
    ...
    public static void verification(SignatureVerifyRequest request) {
        ...
        if (FileTransmissionType.PATH.name().equalsIgnoreCase(request.getFileTransmissionType().name())) {
    ...
    public static void crossPageVerification(AcrossPage acrossPage) {
        ...
        Assert.notNull(acrossPage.getAcrossPagePattern(), StatusConstants.PARAMETER_ERROR, "骑缝章模式不能为空。");
