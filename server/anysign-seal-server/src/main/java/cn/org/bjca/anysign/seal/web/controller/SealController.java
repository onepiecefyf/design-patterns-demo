...
public class SealController {
    ...
    public ObjectRestResponse<SignatureVerifyResponse> pdfVerify(@RequestBody SignatureVerifyRequest request) {
        Assert.notEntity(request.getTransId(), StatusConstants.TRANSID_NOTNULL, null, "验证签章");
        byte[] pdfBty = null;
        switch (request.getFileTransmissionType()) {
            case PATH:
                MoulageConvertWrapper.fileVerification(request.getRequestFileId(), request.getRequestFileToken());
        ...
        List<VerifyMessage> verifyMessages = sealService.sealVerify(request.getAppId(), request.getTransId(), pdfBty, request.getAlg());
        ...
        ObjectRestResponse<SignatureVerifyResponse> response = new ObjectRestResponse<>(signatureVerifyResponse);
        return response;
    }
    ...
    public ObjectRestResponse<SignatureResponse> pdfSignature(@RequestBody SignatureRequest request) {
        ...
        byte[] signedBty = sealService.pdfSignature(pdfBty, request.getAppId(), request.getDeviceId(), request.getSignature(),
        ...
        switch (request.getFileTransmissionType()) {
            case PATH:
                UploadRequestBean uploadRequestBean = UploadRequestBean.build(httpConfig.appId, request.getAppId(), httpConfig.deviceId,
                        httpConfig.version, null, null, Integer.MAX_VALUE, httpConfig.secret, request.getTransId().concat(".pdf"), signedBty);
    ...
    public ObjectRestResponse<SignatureResponse> pdfDigest(@RequestBody DigestRequest request) {
        ...
        switch (request.getFileTransmissionType()) {
            case PATH:
                UploadRequestBean uploadRequestBean = UploadRequestBean.build(httpConfig.appId, request.getAppId(), httpConfig.deviceId,
                        httpConfig.version, null, null, Integer.MAX_VALUE, httpConfig.secret, request.getTransId().concat(".pdf"), pdfDigest.getPdfBty());
                String fileId = systemRequestService.upload(uploadRequestBean);
                signatureResponse.setFileId(fileId);
                break;
            case CONTENT:
    ...
    public ObjectRestResponse<SignatureResponse> pdfMerging(@RequestBody MergingRequest request) {
        ...
        byte[] AllpdfByt = sealService.sealMerging(Base64Utils.base64String2ByteFun(request.getSignData()), pdfBty);
        SignatureResponse signatureResponse = new SignatureResponse();
        switch (request.getFileTransmissionType()) {
            case PATH:
                UploadRequestBean uploadRequestBean = UploadRequestBean.build(httpConfig.appId, request.getAppId(), httpConfig.deviceId,
                        httpConfig.version, null, null, Integer.MAX_VALUE, httpConfig.secret, request.getTransId().concat(".pdf"), AllpdfByt);
                String fileId = systemRequestService.upload(uploadRequestBean);
                signatureResponse.setFileId(fileId);
                break;
            case CONTENT:
