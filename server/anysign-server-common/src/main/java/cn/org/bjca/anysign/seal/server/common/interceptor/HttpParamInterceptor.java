...
public class HttpParamInterceptor implements HandlerInterceptor {
    ...
    @Override
    public boolean preHandle(HttpServletRequest httpServletRequest, HttpServletResponse httpSrvletResponse, Object o) throws Exception {
        ...
        log.info("The IP {} , in time {}, access version is {} , deviceId is {} , appId is {} , url is {}",
                IPUtils.getIPAddress(httpServletRequest), System.currentTimeMillis(), httpServletRequest.getParameter("version"),
                httpServletRequest.getParameter("deviceId"), getAppId(httpServletRequest), getUrl(httpServletRequest)
        );
        MetricsNamePoint metricsNamePoint = new MetricsNamePoint(getUrl(httpServletRequest));
        if (StringUtils.isNotEmpty(metricsNamePoint.getSecondName()) && StringUtils.isNotEmpty(metricsNamePoint.getThirdName())) {
            METRICS.set(MetricsClient.newInstance(metricsNamePoint.getFirstName(), metricsNamePoint.getSecondName(), metricsNamePoint.getThirdName()));
    ...
    private String getAppId(HttpServletRequest httpServletRequest) {
        ...
        if (POST.equals(httpServletRequest.getMethod())) {
            JSONObject parameterMap = JSON.parseObject(new BodyReaderHttpServletRequestWrapper(httpServletRequest).getBodyString(httpServletRequest));
