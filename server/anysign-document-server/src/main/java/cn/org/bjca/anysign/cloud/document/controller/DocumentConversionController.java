...
public class DocumentConversionController extends BaseController {
  ...
  public ObjectRestResponse<DocumentRespMessage> docTransformation(
      @RequestBody DocumentReqMessage reqMessage) {
    // 检测请求参数
    ...
    if (FileTransmissionType.PATH.name().equalsIgnoreCase(fileTransmissionType)) {
      ...
      reqFileBty = systemRequestService
          .download(DownloadRequestBean.build(httpConfig.appId, httpConfig.deviceId,
              httpConfig.version, httpConfig.secret, reqFileId, reqFileToken,
              reqMessage.getTransId()));
    } else {
    ...
    if (isImageFile) { // 图片类型
      ...
      boolean reqScopeFlag = Stream.of(Convert2PicScopeType.values())
          .anyMatch(
              convertScopeType -> convertScopeType.name().equalsIgnoreCase(reqConvertScopeType));
      ...
      if (DocType.PDF.name().equalsIgnoreCase(respFileType)) { // 图片转PDF
        ...
        documentFormatBean
            .setJudgeScope(reqConvertScopeType.equalsIgnoreCase(Convert2PicScopeType.PART.name()));
  ...
  void checkDocumentReqParams(DocumentReqMessage documentReqMessage) {
    ...
    if (FileTransmissionType.PATH.name()
        .equalsIgnoreCase(documentReqMessage.getFileTransmissionType())) {
