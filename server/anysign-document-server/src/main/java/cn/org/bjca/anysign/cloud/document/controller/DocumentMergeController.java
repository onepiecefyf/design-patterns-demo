...
public class DocumentMergeController extends BaseController {
  ...
  public ObjectRestResponse<DocumentRespMessage> docMerging(
    ...
    if (FileTransmissionType.PATH.name().equalsIgnoreCase(fileTransmissionType)) {
      ...
      pdfBtyList = idAndTokenList.parallelStream()
          .map(array -> systemRequestService
              .download(DownloadRequestBean.build(httpConfig.appId, httpConfig.deviceId,
                  httpConfig.version, httpConfig.secret, array[0], array[1],
    ...
    Assert.state(pdfBtyList.parallelStream().noneMatch(this::checkPdfType),
        StatusConstants.DOCUMENT_INCORRECT_PDF_FORMAT,
