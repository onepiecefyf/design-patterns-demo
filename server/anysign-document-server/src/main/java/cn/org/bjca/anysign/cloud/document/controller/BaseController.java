...
public abstract class BaseController {
    ...
    boolean judgeImageFileType(String reqFileType, String respFileType) {
        // 图片类型数组
        String[] picNames = {DocType.JPG.name(), DocType.PNG.name(), DocType.GIF.name(), DocType.TIFF.name()};
        // 是否图片类型 （请求/响应文件类型）
        return Arrays.stream(picNames)
                .anyMatch(picName -> picName.equalsIgnoreCase(reqFileType) || picName.equalsIgnoreCase(respFileType));
    }
    ...
    void checkDocumentReqParams(DocumentReqMessage documentReqMessage) {
        ...
        Assert.notNull(documentReqMessage.getRequestFileType(),
                StatusConstants.DOCUMENT_PARAMETER_ERROR, "The requestFileType is null.", "文档格式化服务", "文档转换");
        Assert.state(judgeFileType(documentReqMessage.getRequestFileType()), StatusConstants.DOCUMENT_PARAMETER_ERROR,
                String.format("The requestFileType %s is wrong.", documentReqMessage.getRequestFileType()), "文档格式化服务", "文档转换");
        Assert.state(judgeFileType(documentReqMessage.getResponseFileType()), StatusConstants.DOCUMENT_PARAMETER_ERROR,
                String.format("The responseFileType %s is wrong.", documentReqMessage.getResponseFileType()), "文档格式化服务", "文档转换");
        Assert.state(judgeTransmissionFileType(documentReqMessage.getFileTransmissionType()), StatusConstants.DOCUMENT_PARAMETER_ERROR,
                String.format("The fileTransmissionType %s is wrong.",  documentReqMessage.getFileTransmissionType()), "文档格式化服务", "文档转换");
    }
     ...
     */
    void buildRespInfo(DocumentReqMessage reqMessage, DocumentRespMessage respMessage, byte[] resultPdfBty) {
        if (PATH.name().equalsIgnoreCase(reqMessage.getFileTransmissionType())) {
            UploadRequestBean uploadRequestBean =
                    UploadRequestBean.build(httpConfig.appId, reqMessage.getAppId(), httpConfig.deviceId, httpConfig.version,
                    reqMessage.getGroup(), reqMessage.getUser(), Integer.MAX_VALUE, httpConfig.secret,
                    reqMessage.getTransId().concat(".").concat(reqMessage.getResponseFileType()), resultPdfBty);
