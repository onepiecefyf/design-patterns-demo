...
public class DocxTemplateController extends BaseController{
    ...
    public ObjectRestResponse<TemplateRespMessage> docGeneration(@RequestBody TemplateReqMessage templateReqMessage) {
        // 模板服务响应消息
        ...
        if (templateImgList != null && !templateImgList.isEmpty()) {
            ...
            imageApprs = templateImgList.stream().map(tempImgMess -> {
                ...
                // 插入图片内容
                imageAppearanceBean.setImageBty(Base64Utils.base64String2ByteFun(tempImgMess.getBase64Img()));
                // 图片显示宽度
                imageAppearanceBean.setWidth(tempImgMess.getDisplayWidth() != 0f?tempImgMess.getDisplayWidth():400f);
                // 图片显示高度
                imageAppearanceBean.setHeight(tempImgMess.getDisplayHeight() != 0f?tempImgMess.getDisplayHeight():400f);
        ...
        if (FileTransmissionType.PATH.name().equalsIgnoreCase(fileTransmissionType)) {
            UploadRequestBean uploadRequestBean = UploadRequestBean.build(
                    ...
                    templateReqMessage.getGroup(), templateReqMessage.getUser(), Integer.MAX_VALUE, httpConfig.secret,
    ...
    void checkTemplateReqParams(TemplateReqMessage templateReqMessage) {
        if (FileTransmissionType.PATH.name().equalsIgnoreCase(templateReqMessage.getFileTransmissionType())) {
