...
import cn.org.bjca.anysign.seal.signature.bean.CustomWaterMarkTool;
...
import cn.org.bjca.seal.esspdf.itext.security.IPDFSign;
import cn.org.bjca.seal.esspdf.itext.security.PDFSM2Sign;
import cn.org.bjca.seal.esspdf.itext.security.PDFSign;
import cn.org.bjca.seal.esspdf.itext.security.TSAClient;
import cn.org.bjca.seal.esspdf.itext.security.TSAClientImpl;
import cn.org.bjca.seal.esspdf.itext.tools.IPKITool;
import cn.org.bjca.seal.esspdf.itext.tools.PDFRSAVerifyTool;
import cn.org.bjca.seal.esspdf.itext.tools.PDFSM2VerifyTool;
import cn.org.bjca.seal.esspdf.itext.tools.RSAPKITool;
import cn.org.bjca.seal.esspdf.itext.tools.SM2PKITool;
...
public class SealServiceImpl implements ISealService {
   ...
   * @Description:
   */
  private byte[] pdfAllSign(byte[] pdfBty, Seal seal, String appId, String deviceId,
      String transId) {
    ...
    // 签发骑缝章
    if (SealPositionType.CROSS_PAGE.ordinal() == seal.getSealPosition().getPositionType()
        .ordinal()) {
   ...
   * @Description:
   */
  private byte[] insertWaterMark(byte[] pdfBty, String waterMarkTemplate, String contents)
      throws Exception {
    // 添加文字水印
    CustomWaterMarkTool waterMarkTool = new CustomWaterMarkTool(pdfBty);
    waterMarkTool
        .addWaterMarkList(CustomWaterMarkTool.parseWaterMarkList(waterMarkTemplate, contents));
  ...
  private byte[] commonSign(byte[] pdfBty, PdfReader pdfReader, int pdfPageNum,
    ...
    try {
      pdfSign = getCommonPDFSign(pdfBty, pdfReader, signCertBty, digestAlg, seal.isTss(),
          seal.getProtectedMode());
    } catch (Exception e) {
    ...
    byte[] result = null;
    try {
      result = sign(hashBty, pdfSign, signCertBty, transId, seal.getVerifyCode(),
          seal.getVerifyCertPolicyId(),
   ...
   * @Description:
   */
  private byte[] acrossPageSign(byte[] pdfBty, PdfReader pdfReader, int pdfPageNum, Seal seal,
      String appId, String deviceId, String transId) {
    ...
    if ("L".equals(acrossPage.getAcrossPagePattern())) {
      ...
      if (pdfPageNum > 1) {
        try {
          subImglist = ImageUtil
              .averageCutImage(subImglist.get(0), DocType.PNG.name(), pdfPageNum - 1, false);
        } catch (Exception e) {
          log.error("acrossPageSign averageCutImage error", e);
          throw new BaseRuntimeException(StatusConstants.ACROSS_AVERAGE_CUTIMAGE_ERROR, null,
              "PDF签章");
        }
      ...
      if (pdfPageNum > 1) {
        try {
          subImglist = ImageUtil
              .averageCutImage(subImglist.get(1), DocType.PNG.name(), pdfPageNum - 1, true);
        } catch (Exception e) {
          log.error("acrossPageSign averageCutImage error", e);
          throw new BaseRuntimeException(StatusConstants.ACROSS_AVERAGE_CUTIMAGE_ERROR, null,
              "PDF签章");
    ...
    for (int k = 1; k <= pdfPageNum; k++) {
      ...
      RectangleBean bean = this
          .getAcrossRectangle(acrossPage.getAcrossPagePattern(), acrossPage.getPosCoord(),
              pdfReader.getPageSizeWithRotation(k), k, cutImageWidth, sealHeight);

      // 支持首页签名和多页签名
      // 骑缝章签名方式，单页签名, 多页签名，默认为单页签名
      // 多页签名
      if (AcrossPageType.MULTI_PAGE.name()
          .equalsIgnoreCase(acrossPage.getAcrossPageType().name())) {
        byte[] signPart = acrossPageSignPart(pdfBty, seal.getProtectedMode(), bean, pdfPageNum,
            signCertBty,
            seal.getKeyId(), seal.getVerifyCode(), seal.getVerifyCertPolicyId(), appId, deviceId,
            transId,
            alg, digestAlg, imgList.get(k - 1), pdfReader, seal.isTss(), seal.isPrintable(),
            seal.getReason(), seal.getLocation(), seal.getContact());
        pdfBty = signPart;
      } else { // 首页签名其他页只加印章图片
        if (k > 1) {
          try {
            pdfBty = insertImgFromPdfGeneral(pdfBty, imgList.get(k - 1), cutImageWidth, sealHeight,
                bean);
          } catch (Exception e) {
        ...
        if (k == pdfPageNum) {
          ...
          bean = this
              .getAcrossRectangle(acrossPage.getAcrossPagePattern(), acrossPage.getPosCoord(),
                  pdfReader.getPageSizeWithRotation(1), 1, cutImageWidth, sealHeight);
          byte[] signPart = acrossPageSignPart(pdfBty, seal.getProtectedMode(), bean, pdfPageNum,
              signCertBty, seal.getKeyId(), seal.getVerifyCode(),
              seal.getVerifyCertPolicyId(), appId, deviceId, transId, alg, digestAlg,
              imgList.get(0), pdfReader, seal.isTss(), seal.isPrintable(),
              seal.getReason(), seal.getLocation(), seal.getContact());
          pdfBty = signPart;
        }
      }
   ...
   * @Description:
   */
  protected byte[] insertImgFromPdfGeneral(byte[] pdfBty, byte[] imgBty, float imgWidth,
      float imgHeight, RectangleBean rectangleBean) throws Exception {
    ByteArrayOutputStream out = null;
    byte[] outPdfBty = null;
    PdfReader pdfReader = null;
    try {
      Image image = Image.getInstance(imgBty);
      Rectangle rectangle = new Rectangle(rectangleBean.getLeft(), rectangleBean.getTop(),
          rectangleBean.getRight(), rectangleBean.getBottom());
  ...
  private byte[] acrossPageSignPart(byte[] pdfBty, ProtectedMode protectedMode, RectangleBean bean,
      int pdfPageNum,
      byte[] signCertBty, String keyId, String verifyCode, String verifyCertPolicyId, String appId,
      String deviceId, String transId, String alg, String digestAlg, byte[] imgBty,
      PdfReader pdfReader,
      boolean isTss, boolean isPrintable, String reason, String location, String contact) {
    //protectedMode 保护模式枚举从0开始，业务上从1开始，枚举需要+1
    IPDFSign pdfSign = null;
    try {
      pdfSign = getAcrossPDFSign(pdfBty, pdfReader, signCertBty, digestAlg, isTss,
          protectedMode.ordinal());
    } catch (Exception e) {
    ...
    byte[] signedBty = null;
    try {
      signedBty = sign(hashBty, pdfSign, signCertBty, transId, verifyCode, verifyCertPolicyId,
          keyId, appId, deviceId, alg, isTss);
    } catch (Exception e) {
      log.error("pdf signature sign error ", e);
   ...
   * @return 返回签名pdf信息
   */
  private byte[] sign(byte[] hashBty, IPDFSign pdfSign, byte[] signCertBty, String transId,
      String verifyCode,
    ...
    if (Alg.RSA.name().equals(keyAlg)) {
      ...
      depositCertSignRequestBean = DepositCertSignRequestBean
          .build(appId, deviceId, SystemConstants.SIGNALGO_HMAC,
              SystemConstants.VERSION_1, data, SystemConstants.DATATYPE_HASH, keyId,
              Alg.SHA256withRSA.name(),
    ...
    signData = this.localSign(hashBty, depositCertSignRequestBean, keyAlg, getPKITool(signCertBty),
        signCertBty, isTSSign);
    if (signData == null) {
   ...
   * @Description:用于PDF签章
   */
  protected byte[] localSign(byte[] hashBty, DepositCertSignRequestBean depositCertSignRequestBean,
      String alg,
  ...
  protected byte[] tssServerSign(byte[] tsReq, String alg, String transId) throws Exception {
    ...
    if (Alg.RSA.name().equals(alg)) {
      GenTsRespRequestBean genTsRespRequestBean = GenTsRespRequestBean
          .build(transId, httpConfig.appId, Alg.SHA256withRSA.name(),
              Base64Utils.byte2Base64StringFun(tsReq));
      jsonObject = systemRequestService.genTsResp(genTsRespRequestBean);
    } else {
      GenTsRespRequestBean genTsRespRequestBean = GenTsRespRequestBean
          .build(transId, httpConfig.appId, Alg.SM3withSM2.name(),
              Base64Utils.byte2Base64StringFun(tsReq));
      jsonObject = systemRequestService.genTsResp(genTsRespRequestBean);
    }
    if (null != jsonObject) {
      ...
      if (Integer.parseInt(StatusConstants.SUCCESS.getStatus()) == status) {
        ...
        log.error("invoke tss micro service error, status code : {},message : {}", status,
  ...
  private float getCutImageWidth(int k, int pdfPageNum, float imageWidth, float sealWidth,
      List<byte[]> splitImages) {
   ...
   * @Description:
   */
  private RectangleBean getAcrossRectangle(String acrossPagePattern, int posCoord,
      Rectangle rectangle, int k, float cutImageWidth, float imageHeight) {
    ...
    RectangleBean bean = createRectangleBean(k, pageWidth - cutImageWidth, yDistanceOrigin,
        pageWidth, yDistanceOrigin + imageHeight);
    // 左骑缝
    if ("L".equals(acrossPagePattern)) { // 左骑缝
      bean = createRectangleBean(k, 0, yDistanceOrigin, cutImageWidth,
          yDistanceOrigin + imageHeight);
    }
    return bean;
  }
   ...
   * @Description:
   */
  protected RectangleBean createRectangleBean(int pageNum, float llx, float lly, float urx,
      float ury) {
  ...
  public PdfDigest pdfDigest(String appId, String transId, byte[] pdfBty,
    ...
    try {
      //protectedMode 保护模式枚举从0开始，业务上从1开始，枚举需要+1
      pdfSign = getCommonPDFSign(pdfBty, pdfReader, Base64Utils.base64String2ByteFun(signCertBty),
          digestAlg,
    ...
    if (SealPositionType.CROSS_PAGE.name().equalsIgnoreCase(sealPositionType.name())) {
      ...
      for (int k = 1; k <= pdfPageNum; k++) {
        ...
        // 多页签名
        if (AcrossPageType.MULTI_PAGE.name()
            .equalsIgnoreCase(acrossPage.getAcrossPageType().toString())) {
          try {
            pdfSign = getAcrossPDFSign(pdfBty, pdfReader,
                Base64Utils.base64String2ByteFun(signCertBty), digestAlg, seal.isTss(),
                protectedMode.ordinal());
          } catch (Exception e) {
            log.error("across page sign getAcrossPDFSign error ", e);
            throw new BaseRuntimeException(StatusConstants.GETPDFSIGN_ERROR, null, "PDF文档摘要");
          ...
          // **********************展示信息
          pdfSign
              .setSignAppearanceInfo(seal.getReason(), seal.getLocation(), seal.getContact(), null);
        } else { // 首页签名其他页只加印章图片
          if (k > 1) {
            try {
              pdfBty = insertImgFromPdfGeneral(pdfBty, imgList.get(k - 1), cutImageWidth,
                  sealHeight, bean);
            } catch (Exception e) {
              throw new BaseRuntimeException("insert image to pdf error ", e);
            }
          ...
          if (k == pdfPageNum) {
            cutImageWidth = this
                .getCutImageWidth(1, pdfPageNum, img.getWidth(), sealWidth, imgList);
            bean = this
                .getAcrossRectangle(acrossPage.getAcrossPagePattern(), acrossPage.getPosCoord(),
                    pdfReader.getPageSizeWithRotation(1), 1, cutImageWidth, sealHeight);
            try {
            ...
            // **********************展示信息
            pdfSign.setSignAppearanceInfo(seal.getReason(), seal.getLocation(), seal.getContact(),
                null);
          }
        }
      }
   ...
   * @param pdfBty PDF
   */
  private byte[] encryptedPdfByte(ProtectedMode protectedMode, byte[] pdfBty)
    ...
    try {
      ...
      stamper.setEncryption(null, null, permissions,
   ...
   * @Description:
   */
  private byte[] getPDF(byte[] clientSignBty, byte[] pdfBty, StringBuffer signTem, String sign)
      throws Exception {
   ...
   */
  protected IPDFSign getCommonPDFSign(byte[] pdfBty, PdfReader pdfReader, byte[] signCertBty,
      String digestAlg, boolean isTss, ProtectedMode protectedMode) throws Exception {
    IPDFSign pdfSign = null;
    int permissions = 0;
   ...
   * @Description:
   */
  protected IPDFSign getAcrossPDFSign(byte[] pdfBty, PdfReader pdfReader, byte[] signCertBty,
      String digestAlg, boolean isTss, int permissions) throws Exception {
    IPDFSign pdfSign = null;
    pdfReader = new PdfReader(pdfBty);
    permissions = 0;
...
//                        message = rsaTool.getVerifyMessage(reader, field, rsaRootCerts, crls, isContainCert);
      ...
      throw new BaseRuntimeException(StatusConstants.SIGNATURE_VERIFY_ERROR, e.getMessage(), null,
          "PDF签章");
    } finally {
      if (reader != null) {
        reader.close();
  ...
  private VerifyMessage getVerifyMessageRSA(String appId, String transId, PdfReader reader,
      SignatureField field) throws Exception {
    ...
    if (pk.getTimeStampDate() != null) {
      message
          .setTimeStampDate((new Timestamp(pk.getTimeStampDate().getTime().getTime())).toString());
    }
  ...
  private boolean VerifyCert(String appId, String transId, byte[] cert) {
    VerifyCertRequesrBean verifyCertRequesrBean = VerifyCertRequesrBean
        .build(appId, Base64Utils.byte2Base64StringFun(cert),
  ...
  private VerifyMessage getVerifyMessageSM2(byte[] pdfBty, SignatureField field) throws Exception {
    ...
    if (null != signCertBytes) {
      String[] signCerts1 = new String[]{
          new String(org.bouncycastle.util.encoders.Base64.encode(signCertBytes))};
  ...
  public String verifySM2Signature(byte[] pdfBty, SignatureField field) throws Exception {
    ...
    byte[] bty2 = ArrayUtil.subArray(pdfBty, field.getByteRange()[2],
        field.getByteRange()[2] + field.getByteRange()[3]);
    ...
    VerifySignRequestBean verifySignRequestBean = VerifySignRequestBean
        .build(httpConfig.appId, httpConfig.deviceId,
            SystemConstants.SIGNALGO_HMAC, httpConfig.version,
            Base64Utils.byte2Base64StringFun("".getBytes()),
            SystemConstants.DATATYPE_HASH, Base64Utils.byte2Base64StringFun(field.getSignCert()),
            Alg.SM3withSM2.name(),
            SystemConstants.SIGNRESULTTYPE_P1, "TRANS_15267543456789",
            Base64Utils.byte2Base64StringFun(signedValue),
            "TestCloudSign");
    PKIToolTemplate pkiToolTemplate = new PKIToolTemplate(systemRequestService);
