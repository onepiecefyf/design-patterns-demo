...
public abstract class BaseImpression extends SealImage implements ICreateImage {
  ...
  private static String shapeString =
      "<ellipse stroke-dashoffset=\"3\" display=\"inline\" fill=\"none\" " +
          "cx=\"%s\" cy=\"%s\" rx=\"%s\"  ry=\"%s\" stroke=\"%s\" stroke-width=\"%s\"/>";
  /**
   * 方章
   */
  private static String shapeRectangleString =
      "<rect stroke-dashoffset=\"3\" display=\"inline\" fill=\"none\" " +
          "x=\"%s\" y=\"%s\" width=\"%s\" height=\"%s\" stroke=\"%s\"\n stroke-width=\"%s\"/>";

  /**
   * 圆章
   */
  private static String shapeCircleleString =
      "<circle stroke-dashoffset=\"3\" display=\"inline\" fill=\"none\" " +
   ...
   * @return 印模信息
   */
  private String getText(String color, int fontSize, String fontFamily, String fontWeight,
  ...
  private String generateText(String id, String text) {
    String template =
        "<textPath xlink:href=\"#" + id + "\" font-stretch=\"condensed\" text-anchor=\"middle\" " +
   ...
   * @return String
   */
  private String shapeRectangleString(int x, int y, int width, int height, String color,
      int stroke) {
  ...
  private String getTexts(TemplateBean templateBean) {
    ...
    for (RiderBean riderBean : templateBean.getRiderBeans()) {
      stringBuffer.append(this.getText(riderBean.getColor(), (int) riderBean.getFontSize(),
          riderBean.getFontFamily(),
  ...
  protected String getSvg(TemplateBean templateBean) {
    ...
    switch (templateBean.getShapeEnum()) {
      ...
      case circle:
        ...
        break;
      case ellipse:
        shape = getShapeString(templateBean.getCenterX(), templateBean.getCenterY(),
            templateBean.getRx(),
