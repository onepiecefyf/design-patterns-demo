...
public class SealImage {
  ...
  private static String template =
      "<svg contentScriptType=\"text/ecmascript\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" xmlns=\"http://www.w3.org/2000/svg\"\n"
          + "     baseProfile=\"full\" zoomAndPan=\"magnify\" contentStyleType=\"text/css\" preserveAspectRatio=\"xMidYMid meet\" width=\""
          + "%s" + "px\" height=\"" + "%s" + "px\"\n"
          + "     version=\"1.0\">" + " %s%s " + "</svg>";
  ...
  private String generateText(String text, String color, int size, String family, String weight,
      String path) {
    String id = UUID.randomUUID().toString();
    String template =
        "<defs><path id=\"" + id + "\" d=\"" + path + "\"/></defs><text style=\"fill:" + color
            + ";\" font-size=\"" + size
            + "\" font-family=\"" + family + "\" font-weight=\"" + weight
            + "\"><textPath xlink:href=\"#" + id
            + "\" text-anchor=\"middle\"  startOffset=\"50%\"  lengthAdjust=\"spacing\">"
            + text + "</textPath></text>";
    return template;
   ...
   */
  private String generateShape(int shape, String color, int stroke, float x, float y,
      float shapeWidth, float shapeHeight) {
    String shapeString = null;
    if (ShapeEnum.circle.ordinal() == shape) {
      shapeString =
          "<circle  stroke-dashoffset=\"3\" display=\"inline\" fill=\"none\" cx=\"" + x + "\" cy=\""
              + y + "\" r=\"" + shapeWidth + "\" stroke=\"" + color + "\"\n"
              + "     stroke-width=\"" + stroke + "\"/>";
    } else if (ShapeEnum.rectangle.ordinal() == shape) {
      shapeString =
          "<rect stroke-dashoffset=\"3\" display=\"inline\" fill=\"none\" x=\"" + x + "\" y=\"" + y
              + "\" width=\"" + shapeWidth + "\"  height=\"" + shapeHeight + "\" stroke=\"" + color
              + "\"\n"
              + "     stroke-width=\"" + stroke + "\"/>";
    } else if (ShapeEnum.ellipse.ordinal() == shape) {
      shapeString =
          "<ellipse stroke-dashoffset=\"3\" display=\"inline\" fill=\"none\" cx=\"" + x + "\" cy=\""
              + y + "\" rx=\"" + shapeWidth + "\"  ry=\"" + shapeHeight + "\" stroke=\"" + color
              + "\"\n"
              + "     stroke-width=\"" + stroke + "\"/>";
    }
    return shapeString;
   ...
   * 获取参数构造形状svg
   */
  private String genSvgParam(float sealWidth, float sealHeight,
      List<Map<String, Object>> textSvgDescs,
   ...
   * 无五角星生成公章
   */
  private String genSvg(float sealWidth, float sealHeight, List<Map<String, Object>> textSvgDescs,
      Map<String, Object> shapeSvgDesc) throws Exception {
   ...
   */
  private String genSvg(float sealWidth, float sealHeight, List<Map<String, Object>> textSvgDescs,
      Map<String, Object> shapeSvgDesc, float xStar, float yStar) throws Exception {
    String textSvgBuffer = this.genSvgParam(sealWidth, sealHeight, textSvgDescs, shapeSvgDesc);
    int shape = (Integer) shapeSvgDesc.get("shape");
    String color = (String) shapeSvgDesc.get("color");
    int stroke = (Integer) shapeSvgDesc.get("stroke");
   ...
   */
  @Deprecated
  public void createSeal(OutputStream outputStream, float sealWidth, float sealHeight,
      List<Map<String, Object>> textSvgDescs, Map<String, Object> shapeSvgDesc) throws Exception {
  ...
  @Deprecated
  public void createSeal(OutputStream outputStream, float sealWidth, float sealHeight,
      List<Map<String, Object>> textSvgDescs, Map<String, Object> shapeSvgDesc, float xStar,
      float yStar) throws Exception {
   ...
   */
  @Deprecated
  public static Map<String, Object> buildShape(int shape, String color, int stroke, float x,
      float y, float shapeWidth, float shapeHeight) {
   ...
   */
  @Deprecated
  public static Map<String, Object> buildText(String text, String color, int size, String family,
      String weight, String path) {
    Map<String, Object> desc = new HashMap<String, Object>(16);
    desc.put("text", text);
    desc.put("color", color);
    desc.put("size", size);
