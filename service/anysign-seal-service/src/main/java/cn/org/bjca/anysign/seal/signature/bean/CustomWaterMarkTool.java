...
public class CustomWaterMarkTool {
   ...
   * @Description:
   */
  public static List<WaterMarkBean> parseWaterMarkList(String wmTemplate, String requestContents)
      throws Exception {
...
//                        // 设置图片的百分比
            ...
            if (StringUtils.isNotBlank(wm.getContent())
                && wm.getContent().length() > MAX_CONTENT_SIZE) {
...
//                        content.setColorFill(Color.getColor(wm.getFontColor()).getRGB());
            content
                .showTextAligned(wm.getAlignment(), wmContent, wm.getPositionX(), wm.getPositionY(),
                    wm.getRotation());
          }
          // 结束
          content.endText();
