...
public class SystemRequestService {
    ...
    public String upload(UploadRequestBean uploadRequestBean) {
        ...
        log.info("request parameter :{} , TransId is {}", uploadRequestBean.toJSONString(httpConfig.secret),
        ...
        try {
            ...
            if (String.valueOf(HttpStatus.SC_OK).equals(jsonObject.get(STATUS).toString())) {
                ...
                String value = jsonObject.getString(STATUS).concat("-").concat(jsonObject.getString(MESSAGE));
    ...
    public JSONObject userApplyCert(DepositCertApplyRequestBean depositCertApplyRequestBean) {
        ...
        log.info("userApplyCert transId is : {} , request parameter :{}",
                depositCertApplyRequestBean.getTransId(), depositCertApplyRequestBean.toJSONString(httpConfig.secret));
     ...
     * "message": "证书签名成功",
     * "data": {
     * "signValue": "AeJsnvxIk9j3d+eozIBEp6QZ9hFUMNMOi6C21XXI1pHpuVet2ps4Abk3iN+6mDgc+ici5dVH82zjYKz92/8TUaW4RIAcKhRt4xtSSO6A7ouZ2h32debMxtjtZmUcK6p91qHpXHZjAOGz0WL+DbYr5j8F24iwwHROVFxze6gxl/M=",
     ...
     * "transId": "TRANS_1526754312345",
     * "requestId": "71846741-7c60-4387-b320-f7d97580996a"
     * }
     * }
     * response format : error
     * { "status": 70000023, "message": "DATATYPE错误" }
     */
    public JSONObject depositCertSign(DepositCertSignRequestBean depositCertSignRequestBean) {
        ...
        log.info("depositCertSign transId is : {} , request parameter :{}",
                depositCertSignRequestBean.getTransId(), depositCertSignRequestBean.toJSONString(httpConfig.secret));
     ...
     * 验签（先验签名值再验证书）
     * response format : success
     * { "status": 200, "message": "证书签名验证成功", "data":{ "transId": "TRANS_15267543456789", "requestId": "7e9a2451-5562-4c22-8746-10a911b2bdcf" } }
     * response format : error
     * { "status": 85006002, "message": "签名值验证不通过", "data": { "transId": "TRANS_15267543456789", "requestId": "a6dea2c4-1a88-429a-8dcf-210af522b7bb" } }
     */
    ...
    public JSONObject genTsReq(GenTsReqRequestBean genTsReqRequestBean) {
        ...
        JSONObject result = HttpUtils.doPost(url, request);
        log.info("genTsReq response data : {}, transId is {}", result.toJSONString(), genTsReqRequestBean.getTransId());
        return result;
    }
    ...
    public JSONObject genTsResp(GenTsRespRequestBean genTsRespRequestBean) {
        ...
        log.info("genTsResp address is : {} , transId is {} , request parameter is {} ", url, genTsRespRequestBean.getTransId(), request);
        JSONObject result = HttpUtils.doPost(url, request);
        log.info("genTsResp response data : {} , transId is {}", result, genTsRespRequestBean.getTransId());
        if (null != result) {
            if (String.valueOf(HttpStatus.SC_OK).equals(result.get(STATUS).toString())) {
                metrics.sr_incrSuccess();
            } else {
