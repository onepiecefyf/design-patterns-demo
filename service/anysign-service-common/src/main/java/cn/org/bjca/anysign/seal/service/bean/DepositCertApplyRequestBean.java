...
public class DepositCertApplyRequestBean extends CommonRequestParam {
   ...
   */
  public static DepositCertApplyRequestBean build(String appId, String deviceId, String signAlgo,
      String version, String keyType,
    ...
    depositCertApplyRequestBean.setAppId(appId).setDeviceId(deviceId).setSignAlgo(signAlgo)
        .setTransId(transId).setVersion(version);
    depositCertApplyRequestBean.setKeyType(keyType).setKeyLength(keyLength)
        .setCertExtensions(certExtensions).setNotBefore(notBefore)
        .setNotAfter(notAfter).setName(name).setIdType(idType).setIdCode(idCode).setMobile(mobile)
        .setVerifyCode(verifyCode).setTemplatePolicyID(templatePolicyID);
